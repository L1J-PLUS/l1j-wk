...
public class L1MonsterInstance extends L1NpcInstance {
	...
	public void receiveDamage(L1Character attacker, int damage) { // 攻撃でＨＰを減らすときはここを使用
		if ((getCurrentHp() > 0) && !isDead()) {
			...
			if (damage >= 0) {
				...
				if ((attacker instanceof L1EffectInstance)) { // Cube's user will get exp and drop
					L1EffectInstance effect = (L1EffectInstance) attacker;
					if (effect.isCube()) 
					setHate(effect.getUser(), damage);
				}
			}
			if (damage > 0) {
	...
	private void giveKarma(L1PcInstance pc) {
		...
		if (karma != 0) {
			...
			if (pc.isInParty()) {
				L1Location location = pc.getLocation();
				L1PcInstance [] partyMember = pc.getParty().getMembers();
				int partycount = pc.getParty().getNumOfMembers();
				for (L1PcInstance member : partyMember) {
					if (member.getLocation().getTileLineDistance(location) > 15) continue;
					int partykarma = (int) (karma * Config.RATE_KARMA / partycount);
					int pcKarmaLevel = member.getKarmaLevel();
