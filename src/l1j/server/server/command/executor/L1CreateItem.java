		...
		try {
			...
			try {
				...
				itemid = ItemTable.getInstance().findItemIdByNameWithoutSpace(
						nameid);
				if (itemid == 0) {
					pc.sendPackets(new S_SystemMessage("can not find item."));
					return;
				}
			}
			...
			if (temp != null) {
						...
						if ((item.getItem().getType2() == 2)
							...
							for (int i = 0; i < enchant; i++) {
								...
								switch (item.getItem().getGrade()) {
									case 0: // 上等
										// 四屬性 +1
										item.setFireMr(item.getFireMr() + 2);
										item.setWaterMr(item.getWaterMr() + 2);
										item.setEarthMr(item.getEarthMr() + 2);
										item.setWindMr(item.getWindMr() + 2);
										// LV6 額外獎勵：體力與魔力回復量 +1
										if (award) {
											item.setHpr(item.getHpr() + 3);
											item.setMpr(item.getMpr() + 3);
										}
										break;
									case 1: // 中等
										// HP +2
										item.setaddHp(item.getaddHp() + 5);
										// LV6 額外獎勵：魔防 +1
										if (award) {
											item.setM_Def(item.getM_Def() + 5);
										}
										break;
									case 2: // 初等
										// MP +1
										item.setaddMp(item.getaddMp() + 5);
										// LV6 額外獎勵：魔攻 +1
										if (award) {
							...
							if (attr.equalsIgnoreCase("earth") || attr.equalsIgnoreCase("1")) {
								if (attLevel > 0 && attLevel <= 3) {
									item.setAttrEnchantKind(1);
									item.setAttrEnchantLevel(attLevel);
								}
							} else if (attr.equalsIgnoreCase("fire") || attr.equalsIgnoreCase("2")) {
								if (attLevel > 0 && attLevel <= 3) {
									item.setAttrEnchantKind(2);
									item.setAttrEnchantLevel(attLevel);
								}
							} else if (attr.equalsIgnoreCase("water") || attr.equalsIgnoreCase("4")) {
								if (attLevel > 0 && attLevel <= 3) {
									item.setAttrEnchantKind(4);
									item.setAttrEnchantLevel(attLevel);
								}
							} else if (attr.equalsIgnoreCase("wind") || attr.equalsIgnoreCase("8")) {
								if (attLevel > 0 && attLevel <= 3) {
				...
				pc.sendPackets(new S_SystemMessage("no such itemid existed."));
			}
		} catch (Exception e) {
			_log.log(Level.SEVERE, e.getLocalizedMessage(), e);
			pc.sendPackets(new S_SystemMessage(
					"Please enter  .item itemid number enchantlvl "));
		}
	}
}
