...
public class L1UltimateBattle {
	...
	private void bossMessage(String msg) {
		for (L1PcInstance pc : getMembersArray()) {
			pc.sendPackets(new S_SystemMessage("The boss for this DM is "+msg+"."));
		}
	}
	...
	class UbThread implements Runnable {
		...
		private void countDown() throws InterruptedException {
			...
			Thread.sleep(1000);
			sendMessage(MSGID_START, "Deathmatch"); // スタート
			removeRetiredMembers();
		}
		...
		public void run() {
			try {
				...
				countDown();
				setNowUb(true);
				if (_ubId ==1) {  // giran dm only
					String [] boss = {"Demon", "Beleth", "Phoenix", "Baphomet", "Giant Ant Queen", "Valakas"};
					if (_pattern >=1 && _pattern <=6) bossMessage(boss[_pattern-1]);
				}
	...
	public String[] makeUbInfoStrings() {
		...
		String teleport = _location.getMap().isEscapable() ? "yes" : "no";
		String res = _location.getMap().isUseResurrection() ? "yes" : "no";
		String pot = "yes";
		String hpr = String.valueOf(_hpr);
		String mpr = String.valueOf(_mpr);
		String summon = _location.getMap().isTakePets() ? "yes" : "no";
		String summon2 = _location.getMap().isRecallPets() ? "yes" : "no";
		_ubInfo = new String[]
