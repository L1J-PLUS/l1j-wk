...
public class L1NpcMakeItemAction extends L1NpcXmlAction {
	...
	private boolean makeItems(L1PcInstance pc, String npcName, int amount) {
		...
		for (L1ObjectAmount<Integer> material : _materials) {
			L1ItemInstance [] itemlist = pc.getInventory().findItemsId(material.getObject());
			L1Item temp = ItemTable.getInstance().getTemplate(material.getObject());
			for (int i=0; i < itemlist.length; i++) {
				if (itemlist[i].getEnchantLevel()>0) {
					pc.sendPackets(new S_SystemMessage("Please remove your +"+itemlist[i].getEnchantLevel()+
							" "+temp.getName()+" from your inventory before crafting."));
					return false;
				}
			}
		}
