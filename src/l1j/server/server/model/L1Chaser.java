...
public class L1Chaser extends TimerTask {
	...
	public void attack() {
		...
		if (_pc.getWeapon() == null) { // 修正空手會出錯的問題
			...
			int healhp = 15 + Random.nextInt(6);
			_pc.setCurrentHp(Math.min(_pc.getCurrentHp()+healhp, _pc.getMaxHp()));
			...
			int healmp = 3 + Random.nextInt(3);
			_pc.setCurrentMp(Math.min(_pc.getCurrentMp()+healmp, _pc.getMaxMp()));
