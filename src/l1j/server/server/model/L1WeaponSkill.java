...
public class L1WeaponSkill {
	...
	static {
		if (Config.USE_INT_PROCS) {
			...
			ProcMap.put(PaagriosHatred, new L1WeaponSkill(
					PaagriosHatred, 12, 0, 0, 0, 0, 0, 7300, 0, false, 2, 0.75));
			ProcMap.put(MaphrsRetribution, new L1WeaponSkill(
					MaphrsRetribution, 15, 0, 0, 0, 0, 0, 1812, 0, false, 1, 1.02));
			...
			ProcMap.put(LongbowOfMoon, new L1WeaponSkill(
					LongbowOfMoon, 10, 0, 0, 0, 0, 0, 6288, 0, true, 0, 0.75));
			ProcMap.put(CSOfMoonlight, new L1WeaponSkill(CSOfMoonlight,
					15, 0, 0, 0, 0, 0, 1810, 0, false, 4, 1.02));
			ProcMap.put(CrystalizedStaff, new L1WeaponSkill(
					CrystalizedStaff, 25, 0, 0, 0, 0, 0, 10, 0, false, 8, 2.63));
			ProcMap.put(PumpkinSword, new L1WeaponSkill(
					PumpkinSword, 8, 0, 0, 0, 0, 0, 2750, 0, false, 1, 1.02));
		}
	}
	...
	public static double getWeaponSkillDamage(final L1PcInstance attacker, 
		...
		ServerBasePacket packet = weaponSkill.isArrowType()
			? new S_UseAttackSkill(attacker, target.getId(), 
	...
	public static double getKiringkuDamage(final L1PcInstance attacker, 
		...
		int damage = 14;
		if (weaponId == 270) damage=16;
		if (weaponId == 309) damage=18;
		int dice = 5;
		int diceCount = 2;
		if (weaponId == 309) diceCount=3;
	...
	private static boolean checkCounterMagic(final L1Character character) {
		...
		int castgfx = SkillsTable.getInstance().findBySkillId(
