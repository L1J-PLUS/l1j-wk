...
*/
	...
	private void addMagicList(L1Character cha, boolean repetition) {
		...
		// 骷髏毀壞持續時間另外處理 removed BONE_BREAK HERE
		...
		try {
			...
			for (Iterator<TargetStatus> iter = _targetList.iterator(); iter.hasNext();) {
				...
				switch(_skillId) {
					...
					int boneTime = (random1.nextInt(10) + 6) * 100;
					_boneBreakDuration = boneTime;
					int intbonus = _user.getInt() - 12;
					if (intbonus > 0) {
						_boneBreakDuration = _boneBreakDuration + (intbonus * 40);
					}
					if (_boneBreakDuration < 1000) {
						_boneBreakDuration = 1000;
					}
					if (_boneBreakDuration > 2500) {
						_boneBreakDuration = 2500;
					}
